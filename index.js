...
const argv = require("yargs")
                .options({
                    ...
                    p: {
                        alias: "commentPrefix",
                        describe: "Customize comment prefix",
                        type: "string",
                        default: "→",
                    },
...
if (useFile) {
    ...
    const errors = files.map(filePath => ({
        path: filePath,
        errors: connoisseur(fs.readFileSync(filePath, "utf8"), filePath, { commentPrefix }),
    }))
    ...
    process.stdin.pipe(concat({ encoding: "string" }, text => {
        const errors = {
            path: "STDIN",
            errors: connoisseur(text, "STDIN", { commentPrefix }),
        };
